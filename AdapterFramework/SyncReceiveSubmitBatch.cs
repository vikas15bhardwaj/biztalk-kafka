...
namespace AdapterFramework
{
    public class SyncReceiveSubmitBatch : ReceiveBatch
    {
        ...
        public SyncReceiveSubmitBatch(IBTTransportProxy transportProxy, ControlledTermination control, int depth, string traceId)
            : this(transportProxy, control, new ManualResetEvent(false), depth, traceId) { }

        private SyncReceiveSubmitBatch(IBTTransportProxy transportProxy, ControlledTermination control,
                                        ManualResetEvent submitComplete, int depth, string traceId)
            : base(transportProxy, control, submitComplete, depth, traceId)
        {
