...
namespace AdapterFramework
{
    public class ControlledTermination : IDisposable
    {
        ...
        {
            bool result;
            TraceHelper.Logger.TraceInfo($"ControlledTermination: Terminate ${this.activityCount}");

            lock (this)
            {
                TraceHelper.Logger.TraceInfo("ControlledTermination: Terminate");
                this.terminate = true;
                result = (this.activityCount == 0);
            }

            TraceHelper.Logger.TraceInfo($"ControlledTermination: Terminate ${this.activityCount} {result}");
