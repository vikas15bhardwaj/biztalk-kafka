...
namespace AdapterFramework
{
    ...
    public class Batch : IBTBatchCallBack, System.IDisposable
    {
        ...
        public void BatchComplete (Int32 hrStatus, Int16 nOpCount, BTBatchOperationStatus[] pOperationStatus, System.Object vCallbackCookie)
        {
            ...
            TraceHelper.Logger.TraceInfo($"{TraceId}: Batch.BatchComplete starts");
            if (0 != hrStatus)
            {
            ...
            TraceHelper.Logger.TraceInfo($"{TraceId}: Batch.BatchComplete StartBatchComplete {hrStatus}");
            ...
            if (hrStatus < 0 || this.makeSuccessCall)
            {
                ...
                TraceHelper.Logger.TraceInfo($"{TraceId}: Batch.BatchComplete StartProcessFailures");

                StartProcessFailures();

                foreach (BTBatchOperationStatus status in pOperationStatus)
                {
                    TraceHelper.Logger.TraceInfo($"{TraceId}: Batch.BatchComplete StartProcessFailures {status.OperationType.ToString()} {status.Status} {status.MessageCount}");
                ...
                TraceHelper.Logger.TraceInfo($"{TraceId}: Batch.BatchComplete EndProcessFailures");
                EndProcessFailures();
            } // end if
            TraceHelper.Logger.TraceInfo($"{TraceId}: Batch.BatchComplete EndBatchComplete");
        ...
        public Batch (IBTTransportProxy transportProxy, bool makeSuccessCall, string traceId)
        {
            this.hrStatus = -1;
        ...
        public IBTDTCCommitConfirm Done (object transaction)
        {
            ...
            TraceHelper.Logger.TraceInfo($"{TraceId}: Entering Batch.Done");
            try
            {
                if (workToBeDone)
                {
            ...
            finally
            {
                ...
                TraceHelper.Logger.TraceInfo($"{TraceId}: Leaving Batch.Done");
