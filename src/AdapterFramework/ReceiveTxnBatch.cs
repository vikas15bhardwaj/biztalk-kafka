...
namespace AdapterFramework
{
	...
	public sealed class AbortOnFailureReceiveTxnBatch : TxnBatch
	{
        ...
        public AbortOnFailureReceiveTxnBatch(IBTTransportProxy transportProxy, ControlledTermination control, CommittableTransaction transaction, ManualResetEvent orderedEvent, TxnAborted txnAborted, string traceId) : base(transportProxy, control, transaction, orderedEvent, false, traceId)
        { 
			this.txnAborted = txnAborted;
		}
        public AbortOnFailureReceiveTxnBatch(IBTTransportProxy transportProxy, ControlledTermination control, IDtcTransaction comTxn, CommittableTransaction transaction, ManualResetEvent orderedEvent, TxnAborted txnAborted, string traceId)
            : base(transportProxy, control, comTxn, transaction, orderedEvent, false, traceId)
        {
            this.txnAborted = txnAborted;
        }
	...
	public sealed class AbortOnAllFailureReceiveTxnBatch : TxnBatch
	{
        ...
        public AbortOnAllFailureReceiveTxnBatch(IBTTransportProxy transportProxy, ControlledTermination control, CommittableTransaction transaction, ManualResetEvent orderedEvent, StopProcessing stopProcessing, string traceId) : base(transportProxy, control, transaction, orderedEvent, false, traceId)
        {
	...
	{
        public SingleMessageReceiveTxnBatch(IBTTransportProxy transportProxy, ControlledTermination control, CommittableTransaction transaction, ManualResetEvent orderedEvent, string traceId) : base(transportProxy, control, transaction, orderedEvent, true, traceId)
        { }
		protected override void StartProcessFailures ()
		{
			if (!this.OverallSuccess)
			{
				this.innerBatch = new AbortOnFailureReceiveTxnBatch(this.TransportProxy, this.control, this.comTxn, this.transaction, this.orderedEvent, null, this.TraceId);
