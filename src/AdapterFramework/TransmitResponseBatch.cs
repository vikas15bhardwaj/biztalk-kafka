...
namespace AdapterFramework
{
    ...
    public sealed class TransmitResponseBatch : Batch
    {
        ...
        public TransmitResponseBatch(IBTTransportProxy transportProxy, AllWorkDoneDelegate allWorkDoneDelegate, string traceId)
            : base(transportProxy, true, traceId)
        {
            TraceHelper.Logger.TraceInfo($"{TraceId}: TransmitResponseBatch");
        ...
        {
            TraceHelper.Logger.TraceInfo($"{TraceId}: TransmitResponseBatch.SubmitResponseMessage");
        ...
        {
            TraceHelper.Logger.TraceInfo($"{TraceId}: TransmitResponseBatch.Resubmit");
        ...
        {
            TraceHelper.Logger.TraceInfo($"{TraceId}: TransmitResponseBatch.Resubmit2");
        ...
        {
            TraceHelper.Logger.TraceInfo($"{TraceId}: TransmitResponseBatch.StartProcessFailures");
            if (this.batchFailed)
            {
                // Retry should happen outside the transaction scope
                this.batch = new TransmitResponseBatch(this.TransportProxy, this.allWorkDoneDelegate, this.TraceId);
        ...
        {
            TraceHelper.Logger.TraceInfo($"{TraceId}: TransmitResponseBatch.EndProcessFailures");
            if (this.batch != null)
            {
        ...
        {
            TraceHelper.Logger.TraceInfo($"{TraceId}: TransmitResponseBatch.EndBatchComplete");
        ...
        {
            TraceHelper.Logger.TraceInfo($"{TraceId}: TransmitResponseBatch.SubmitSuccess {this.batchFailed}");
            if (this.batchFailed)
            {
