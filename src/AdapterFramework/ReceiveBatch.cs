...
namespace AdapterFramework
{
    ...
    {
        public ReceiveBatch (IBTTransportProxy transportProxy, ControlledTermination control, ManualResetEvent orderedEvent, int depth, string traceId) : base(transportProxy, true, traceId)
        {
        ...
        public ReceiveBatch(IBTTransportProxy transportProxy, ControlledTermination control, ReceiveBatchCompleteHandler callback, int depth, string traceId) : base(transportProxy, true, traceId)
        {
            this.control = control;
        ...
        protected override void StartProcessFailures ()
        {
            ...
            if (!this.OverallSuccess && this.depth > 0)
            {
                ...
                {
                    this.innerBatch = new ReceiveBatch(this.TransportProxy, this.control, this.orderedEvent, this.depth - 1, this.TraceId);
                    this.innerBatch.ReceiveBatchComplete = this.ReceiveBatchComplete;
                }
                else
                {
                    this.innerBatch = new ReceiveBatch(this.TransportProxy, this.control, this.ReceiveBatchComplete, this.depth - 1, this.TraceId);
                }
